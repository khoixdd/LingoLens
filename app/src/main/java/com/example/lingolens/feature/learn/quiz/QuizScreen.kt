            ...
            state.isEmpty -> {
                Box(
                    ...
                    EmptyState(
                        ...
                        message = "Add words in your Notebook to generate a quiz session!",
                    )
                }
            }
            else -> {
                LazyColumn(
                        ...
                        LinearProgressIndicator(
                            progress = {
                                if (state.totalQuestions > 0) {
                                    (state.questionIndex + 1) / state.totalQuestions.toFloat()
                                } else 0f
                            },
                            modifier = Modifier
                                .fillMaxWidth()
                    ...
                    items(state.options.size) { index ->
                        QuizOption(
                            ...
                            state = state,
                            onClick = { onAction(QuizAction.SelectAnswer(index)) },
                        )
                    }
                    if (state.answerState == QuizAnswerState.Correct || state.answerState == QuizAnswerState.Incorrect) {
                        item {
                    ...
                    item {
                        ...
                        Button(
                            onClick = { onAction(if (checked) QuizAction.Next else QuizAction.CheckAnswer) },
                            enabled = state.selectedIndex != null,
                            modifier = Modifier.fillMaxWidth(),
                        ) {
                            Text(if (checked) "Next" else "Check answer")
                        }
                    }
                }
            }
    ...
    LingoLensTheme(darkTheme = false) {
        QuizScreen(
            QuizUiState(
                isLoading = false,
                word = "ubiquitous",
                options = listOf("hiếm hoi", "khó khăn", "phổ biến", "tạm thời"),
                correctIndex = 2,
                selectedIndex = 2,
